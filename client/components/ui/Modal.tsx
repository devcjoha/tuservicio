...
export function Modal({ isOpen, onClose, title, variant = "form", children }: Props) {
  ...
  return (
    <div
      className="modal fixed inset-0 z-50 flex flex-row items-center justify-center bg-opacity-10 bg-slate-100/10 backdrop-blur-xs"
      role="dialog"
      aria-modal="true"
    >
      <div
        className={`flex flex-col rounded-lg shadow-lg w-full max-w-lg p-4 relative ${variantStyles[variant]} ${variantAnimations[variant]}`}
      >
        <button onClick={onClose} aria-label="Cerrar modal" className="flex justify-end rounded hover:bg-gray-100">
          <X className="w-5 h-5" />
        </button>
        <div className="flex justify-left items-center p-3 gap-3">
          <div className="">
            {icon[variant]}
          </div>
